...
export function listaPrestamos() {
    ...
    arrayPrestamos.forEach(elemento => {
        ...
        li.innerHTML = `${elemento.id},  ${elemento.titulo} <span class="badge bg-secondary">${elemento.editorial}</span>`;
        ul.appendChild(li);
    });
    return ul;
...
export function imgPrestamos() {
    ...
    imagen.alt = 'Imagen de préstamos';
    return imagen;
}
...
export function listaSocios() {
    ...
    arraySocios.forEach(elemento => {
        let li = document.createElement('li');
        li.classList.add('list-group-item');
        li.innerHTML = `${elemento.nombre} <span class="badge bg-secondary">${elemento.mail}</span>`;
        ul.appendChild(li);
    });
    return ul;
}
